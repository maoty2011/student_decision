...
class ContextualBandit(object):
	...
	def add_result_seg(self, population, group):
		...
		raw_feedback = THEkpi.THEKPI(population, group)
		#raw_feedback = KPIs_segmented.KPI4(population, group)
	...
	def send_recommendation(self, current_user=None, method='Gaussian',allow_stop=False):#method='disjointUCB','Gaussian'
		...
		for trial_id in range(self.num_arms):
			...
			#print(loc_mean, loc_sd)
			sampled_p += [dist.rvs()]
		#print(sampled_p)
		recommended_arm = sampled_p.index( max(sampled_p) )
		#print ('Max',max(sampled_p),allow_stop)
		if allow_stop:
			#print 'Max',max(sampled_p)
			# allow us to stop sending offers when KPI < 1
			if (max(sampled_p) < 1):
				#print("!!!!!!!")
				return None
	...
	def recommendation_to_csv(self,deliveries_file_name=None,delimiter='|',control_fraction=0.2, allow_stop=False):
			...
			for users in self.cluster_dict.keys():
				if (self.cluster_dict[users]==[i]):
					...
					#only give offer to current target group people at a rate
						rec_offer = self.send_recommendation(users,allow_stop=allow_stop) # None if we stop sending offers
						if (rec_offer != None):
							current_rec = rec_offer.id
							deliveries.append((users, current_rec))
							counter[i][loc_offer_list.index(current_rec)] += 1
						else:
							counter[i][self.num_arms] += 1 # force into control group
					else:
