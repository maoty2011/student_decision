...
#    print(data_write_path,data_read_path)
    ...
    #make it the real population
    population = experimentr.main(args)
    ...
    table=myBandit.recommendation_to_csv(deliveries_file_name = deliveries_file_name, allow_stop=False)
    shutil.copy(deliveries_file_name, deliveries_log_file_name)
...
#        	dynamic_control = 0.9
        table = myBandit.recommendation_to_csv(deliveries_file_name = deliveries_file_name, control_fraction=dynamic_control, allow_stop=False)
...
def get_args():
    ...
    parser.add_argument("--data-path", default="zdata", help="data path file name")
    parser.add_argument("--n-proc", default=1, help="number of Processes to use for simulation")
    parser.add_argument("--repeat", default=10, help="number of periods to repeat")
